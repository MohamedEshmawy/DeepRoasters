...
import os
import sys
# sys.path.append('/home/mohamed/Desktop/Image_Processing/CVC/DeepRoasters')
...
if __name__ == '__main__':
    ...
    if camera:
        ...
        # Parse Image file
        img_files = sorted(glob.glob(join(base_path, '*.jp*')))
        ...
        for f, im in enumerate(ims):
            if f == 0:  # init
                ...
                if enable_mask:
                    ...
                    state = siamese_track(state, im, mask_enable=False, refine_enable=False, device=device)  # track
                    cv2.rectangle(im, tuple((state['target_pos']-state['target_sz']//2).astype(np.int)),
                            tuple((state['target_pos']+state['target_sz']//2).astype(np.int)), (255, 255, 0) , 3)
